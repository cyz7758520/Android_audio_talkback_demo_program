...
public class MyClntMediaPocsThrd extends ClntMediaPocsThrd
{
    ...
    @Override public void UserClntMediaPocsThrdDstoy()
    {
        ...
        m_MainActPt.SendVdoInptOtptSurfaceViewDstoyMsg( m_VdoInptPt.m_DvcPt.m_PrvwSurfaceViewPt );
    }
    ...
    @Override public void UserTkbkClntCnctSts( int CurCnctSts )
    {
        ...
        {
            m_MainActPt.SendMainActMsg( MainAct.MainActMsgTyp.ClntLstModifyItem, "等待远端接受连接", null );
        }
        else if( CurCnctSts < ClntMediaPocsThrd.CnctSts.Wait )
        {
            m_MainActPt.SendMainActMsg( MainAct.MainActMsgTyp.ClntLstModifyItem, "第" + -CurCnctSts + "次连接", null );
        }
        else if( CurCnctSts == ClntMediaPocsThrd.CnctSts.Cnct )
        {
            m_MainActPt.SendMainActMsg( MainAct.MainActMsgTyp.ClntLstModifyItem, "已连接", null );
        }
        else if( CurCnctSts == ClntMediaPocsThrd.CnctSts.Tmot )
        {
            m_MainActPt.SendMainActMsg( MainAct.MainActMsgTyp.ClntLstModifyItem, "异常断开", null );
        }
        else if( CurCnctSts == ClntMediaPocsThrd.CnctSts.Dsct )
        {
            m_MainActPt.SendMainActMsg( MainAct.MainActMsgTyp.ClntLstModifyItem, "已断开", null );
        }
    }
    ...
    {
        m_MainActPt.SendVdoInptOtptSurfaceViewSetTitleMsg( m_VdoInptPt.m_DvcPt.m_PrvwSurfaceViewPt, "视频输入预览" + MyTkbkIdx );
    }
    ...
    @Override public void UserTkbkClntLclTkbkMode( int OldLclTkbkMode, int NewLclTkbkMode )
    {
        ...
        if( ( ( CheckBox ) m_MainActPt.m_StngLyotViewPt.findViewById( R.id.IsSaveAdoVdoInptOtptToAviFileCkBoxId ) ).isChecked() )
        {
            ...
            try
            {
                m_MainActPt.m_MyClntMediaPocsThrdPt.SetIsSaveAdoVdoInptOtptToAviFile(
                        0,
                        p_FullPathStrPt,
    ...
    {
        DelAdoOtptStrm( 1, TkbkInfoPt.m_TkbkIdx );

        for( VdoOtpt.Strm p_StrmPt : m_VdoOtptPt.m_StrmCntnrPt )
        {
            if( p_StrmPt.m_Idx == TkbkInfoPt.m_TkbkIdx )
            {
                m_MainActPt.SendVdoInptOtptSurfaceViewDstoyMsg( p_StrmPt.m_DvcPt.m_DspySurfaceViewPt );
                break;
            }
        }
        VdoOtptDelStrm( 1, TkbkInfoPt.m_TkbkIdx );
    }
    ...
    void SetToUseVdoInpt()
    {
        ...
        if( ( ( RadioButton ) m_MainActPt.m_StngLyotViewPt.findViewById( R.id.UseVdoFrmSzPrsetRdBtnId ) ).isChecked() ) //如果要使用预设的帧的大小。
        {
            SetVdoInpt(
                    ...
                    m_MainActPt.SendVdoInptOtptSurfaceViewInitMsg( "视频输入预览" + m_TkbkClntPt.m_MyTkbkIdx ) );
        }
        else //如果要使用其他的帧的大小。
        {
            try
    ...
    void SetToUseVdoOtptStrm( int TkbkIdx )
    {
        ...
        VdoOtptSetStrm(
                0,
                TkbkIdx,
                m_MainActPt.SendVdoInptOtptSurfaceViewInitMsg( "视频输出显示" + TkbkIdx ) );
