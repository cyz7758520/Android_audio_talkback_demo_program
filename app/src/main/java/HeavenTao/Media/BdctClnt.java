...
public class BdctClnt //广播客户端。
{
        ...
        Out:
        {
            if( CnctInfoPt.m_IsInit != 0 )
            {
                ...
                if( ( CnctInfoPt.m_IsRecvExitPkt == 0 ) && ( CnctInfoPt.m_CurCnctSts == ClntMediaPocsThrd.CnctSts.Cnct ) ) //如果未接收退出包，且当前连接状态为已连接。
                {
                    ...
                    m_ClntMediaPocsThrdPt.m_TmpBytePt[ 1 ] = ( byte ) CnctInfoPt.m_MyTkbkIdx; //设置对讲索引。
                    if( ( ( CnctInfoPt.m_IsTcpOrAudpPrtcl == 0 ) && ( CnctInfoPt.m_TcpClntSoktPt.SendApkt( m_ClntMediaPocsThrdPt.m_TmpBytePt, 2, ( short ) 0, 1, 0, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 ) ) ||
                            ( ( CnctInfoPt.m_IsTcpOrAudpPrtcl == 1 ) && ( m_ClntMediaPocsThrdPt.m_AudpClntSoktPt.SendApkt( CnctInfoPt.m_AudpClntCnctIdx, m_ClntMediaPocsThrdPt.m_TmpBytePt, 2, 1, 1, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 ) ) )
                    {
                        String p_InfoStrPt = "客户端媒体处理线程：对讲客户端：发送退出包成功。对讲索引：" + CnctInfoPt.m_MyTkbkIdx + "。总长度：2。";
                    ...
                    {
                        String p_InfoStrPt = "客户端媒体处理线程：对讲客户端：发送退出包失败。对讲索引：" + CnctInfoPt.m_MyTkbkIdx + "。总长度：2。原因：" + m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt.GetStr();
                ...
                if( ( CnctInfoPt.m_IsRecvExitPkt == 0 ) && ( CnctInfoPt.m_CurCnctSts == ClntMediaPocsThrd.CnctSts.Tmot ) ) //如果未接收到退出包，且当前连接状态为异常断开，就重连。
                {
    ...
    public int CnctInfoSendTkbkModePkt( CnctInfo p_CnctInfoPt, int LclTkbkMode )
    {
        ...
        Out:
        {
            ...
            m_ClntMediaPocsThrdPt.m_TmpBytePt[ 1 ] = ( byte )p_CnctInfoPt.m_MyTkbkIdx;
            m_ClntMediaPocsThrdPt.m_TmpBytePt[ 2 ] = ( byte )LclTkbkMode;
            if( CnctInfoSendPkt( p_CnctInfoPt, m_ClntMediaPocsThrdPt.m_TmpBytePt, 3, 1, 1, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
            {
            ...
            {
                String p_InfoStrPt = "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoPt.m_Idx + "：发送对讲模式包失败。原因：" + m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt.GetStr();
                if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, p_InfoStrPt );
                m_ClntMediaPocsThrdPt.UserShowLog( p_InfoStrPt );
            }
    ...
    public void AllCnctSendAdoPkt( byte PktPt[], long PktLenByt, int Times, int IsRlab )
    {
        ...
        Out:
        {
            ...
            {
                p_CnctInfoTmpPt = m_CnctInfoCntnrPt.get( p_CnctInfoLstIdx );
                if( ( p_CnctInfoTmpPt.m_IsInit != 0 ) && ( p_CnctInfoTmpPt.m_CurCnctSts == ClntMediaPocsThrd.CnctSts.Cnct ) && ( p_CnctInfoTmpPt.m_MyTkbkIdx != -1 ) )
                {
                    //设置对讲索引。
                    PktPt[ 1 ] = ( byte ) p_CnctInfoTmpPt.m_MyTkbkIdx;
            ...
            for( int p_CnctInfoLstIdx = 0; p_CnctInfoLstIdx < m_CnctInfoCntnrPt.size(); p_CnctInfoLstIdx++ )
            {
                ...
                if( p_CnctInfoTmpPt.m_IsInit != 0 )
                {
                        ...
                        if( p_CnctInfoTmpPt.m_IsTcpOrAudpPrtcl == 0 ) //用本端Tcp协议客户端套接字连接远端Tcp协议服务端套接字。
                        {
                            ...
                            TcpClntSoktCnctOut:
                            {
                                if( p_CnctInfoTmpPt.m_TcpClntSoktPt == null ) //如果未初始化本端Tcp协议客户端套接字。
                                {
                                    ...
                                    if( p_CnctInfoTmpPt.m_TcpClntSoktPt.Init( p_RmtNodeAddrFamly, p_CnctInfoTmpPt.m_RmtNodeNameStrPt, p_CnctInfoTmpPt.m_RmtNodeSrvcStrPt, null, null, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) != 0 ) //如果初始化本端Tcp协议客户端套接字，并连接远端Tcp协议服务端套接字失败。
                                    {
                                ...
                                if( p_CnctInfoTmpPt.m_TcpClntSoktPt.WaitCnct( ( short ) 0, m_ClntMediaPocsThrdPt.m_TmpHTIntPt, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 ) //如果等待本端Tcp协议客户端套接字连接远端Tcp协议服务端套接字成功。
                                {
                                    ...
                                    else if( m_ClntMediaPocsThrdPt.m_TmpHTIntPt.m_Val == TcpClntSokt.TcpCnctStsCnct ) //如果连接成功。
                                    {
                                        ...
                                        if( p_CnctInfoTmpPt.m_TcpClntSoktPt.GetLclAddr( null, m_ClntMediaPocsThrdPt.m_LclNodeAddrPt, m_ClntMediaPocsThrdPt.m_LclNodePortPt, 0, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) != 0 )
                                        {
                                        ...
                                        if( p_CnctInfoTmpPt.m_TcpClntSoktPt.GetRmtAddr( null, m_ClntMediaPocsThrdPt.m_RmtNodeAddrPt, m_ClntMediaPocsThrdPt.m_RmtNodePortPt, 0, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) != 0 )
                                        {
                            ...
                            AudpClntSoktCnctOut:
                            {
                                if( m_ClntMediaPocsThrdPt.m_AudpClntSoktPt == null ) //如果未初始化本端高级Udp协议客户端套接字。
                                {
                                    ...
                                    if( m_ClntMediaPocsThrdPt.m_AudpClntSoktPt.Init( p_RmtNodeAddrFamly, null, null, ( short )0, ( short )5000, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 ) //如果初始化本端高级Udp协议客户端套接字成功。
                                    {
                                        ...
                                        String p_InfoStrPt = "客户端媒体处理线程：广播客户端：初始化本端高级Udp协议客户端套接字[" + m_ClntMediaPocsThrdPt.m_LclNodeAddrPt.m_Val + ":" + m_ClntMediaPocsThrdPt.m_LclNodePortPt.m_Val + "]成功。";
                                        if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, p_InfoStrPt );
                                        m_ClntMediaPocsThrdPt.UserShowLog( p_InfoStrPt );
                                    }
                                ...
                                if( p_CnctInfoTmpPt.m_AudpClntCnctIdx == -1 )
                                {
                                    ...
                                    if( m_ClntMediaPocsThrdPt.m_AudpClntSoktPt.Cnct( p_RmtNodeAddrFamly, p_CnctInfoTmpPt.m_RmtNodeNameStrPt, p_CnctInfoTmpPt.m_RmtNodeSrvcStrPt, m_ClntMediaPocsThrdPt.m_TmpHTLongPt, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) != 0 ) //如果用本端高级Udp协议客户端套接字连接远端高级Udp协议服务端套接字失败。
                                    {
                                    ...
                                    p_CnctInfoTmpPt.m_AudpClntCnctIdx = m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val; //设置本端高级Udp协议客户端连接索引。
                                }

                                if( m_ClntMediaPocsThrdPt.m_AudpClntSoktPt.WaitCnct( p_CnctInfoTmpPt.m_AudpClntCnctIdx, ( short )0, m_ClntMediaPocsThrdPt.m_TmpHTIntPt, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 ) //循环等待本端高级Udp协议套接字连接远端成功。
                                {
                                    ...
                                    {
                                        if( m_ClntMediaPocsThrdPt.m_AudpClntSoktPt.GetRmtAddr( p_CnctInfoTmpPt.m_AudpClntCnctIdx, null, m_ClntMediaPocsThrdPt.m_RmtNodeAddrPt, m_ClntMediaPocsThrdPt.m_RmtNodePortPt, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) != 0 )
                                        {
                    ...
                    if( ( p_CnctInfoTmpPt.m_IsRqstDstoy == 0 ) && ( p_CnctInfoTmpPt.m_CurCnctSts == ClntMediaPocsThrd.CnctSts.Cnct ) ) //如果该连接未请求销毁，且当前连接状态为已连接。
                    {
                        RecvPktOut:
                        {
                            if( ( ( p_CnctInfoTmpPt.m_IsTcpOrAudpPrtcl == 0 ) && ( p_CnctInfoTmpPt.m_TcpClntSoktPt.RecvApkt( m_ClntMediaPocsThrdPt.m_TmpBytePt, m_ClntMediaPocsThrdPt.m_TmpBytePt.length, m_ClntMediaPocsThrdPt.m_TmpHTLongPt, ( short ) 0, 0, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 ) ) ||
                                ( ( p_CnctInfoTmpPt.m_IsTcpOrAudpPrtcl == 1 ) && ( m_ClntMediaPocsThrdPt.m_AudpClntSoktPt.RecvApkt( p_CnctInfoTmpPt.m_AudpClntCnctIdx, m_ClntMediaPocsThrdPt.m_TmpBytePt, m_ClntMediaPocsThrdPt.m_TmpBytePt.length, m_ClntMediaPocsThrdPt.m_TmpHTLongPt, null, ( short ) 0, m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 ) ) )
                            {
                                if( m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val != -1 ) //如果用本端套接字接收一个连接的远端套接字发送的数据包成功。
                                {
                                    if( m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val == 0 ) //如果数据包的数据长度为0。
                                    {
                                        if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收数据包。长度为" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "，表示没有数据，无法继续接收。" );
                                        break RecvPktOut;
                                    }
                                    else if( m_ClntMediaPocsThrdPt.m_TmpBytePt[ 0 ] == ( byte ) ClntMediaPocsThrd.PktTyp.TkbkIdx ) //如果是对讲索引包。
                                    {
                                        if( m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val < 1 + 1 ) //如果对讲模式包的长度小于1 + 1，表示没有对讲索引。
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收对讲索引包。长度为" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "小于1 + 1，表示没有对讲索引，无法继续接收。" );
                                            break RecvPktOut;
                                        }
                                        ...
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收其他对讲索引包。对讲索引：" + m_ClntMediaPocsThrdPt.m_TmpBytePt[ 1 ] + "。" );
                                        }
                                    ...
                                    else if( m_ClntMediaPocsThrdPt.m_TmpBytePt[ 0 ] == ( byte ) ClntMediaPocsThrd.PktTyp.TkbkMode ) //如果是对讲模式包。
                                    {
                                        ...
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收对讲模式包。长度为" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "小于1 + 1 + 1，表示没有对讲索引和对讲模式，无法继续接收。" );
                                            break RecvPktOut;
                                        }
                                        ...
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收对讲模式包。对讲模式为" + m_ClntMediaPocsThrdPt.m_TmpBytePt[ 2 ] + "不正确，无法继续接收。" );
                                            break RecvPktOut;
                                        }

                                        if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收对讲模式包。对讲索引：" + m_ClntMediaPocsThrdPt.m_TmpBytePt[ 1 ] + "。对讲模式：" + ClntMediaPocsThrd.m_TkbkModeStrArrPt[ m_ClntMediaPocsThrdPt.m_TmpBytePt[ 2 ] ] + "。" );
                                    }
                                    else if( m_ClntMediaPocsThrdPt.m_TmpBytePt[ 0 ] == ( byte ) ClntMediaPocsThrd.PktTyp.AdoFrm ) //如果是音频输出帧包。
                                    {
                                        ...
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收音频输出帧包。长度为" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "小于1 + 1 + 4，表示没有对讲索引和音频输出帧时间戳，无法继续接收。" );
                                            break RecvPktOut;
                                        }
                                        ...
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收有语音活动的音频输出帧包。音频输出帧时间戳：" + p_TmpInt + "。总长度：" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "。" );
                                        }
                                        else //如果该音频输出帧为无语音活动。
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收无语音活动的音频输出帧包。音频输出帧时间戳：" + p_TmpInt + "。总长度：" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "。" );
                                        }
                                    }
                                    else if( m_ClntMediaPocsThrdPt.m_TmpBytePt[ 0 ] == ( byte ) ClntMediaPocsThrd.PktTyp.VdoFrm ) //如果是视频输出帧包。
                                    {
                                        ...
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收视频输出帧包。长度为" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "小于1 + 1 + 4，表示没有对讲索引和视频输出帧时间戳，无法继续接收。" );
                                            break RecvPktOut;
                                        }
                                        ...
                                        if( m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val > 1 + 4 ) //如果该视频输出帧为有图像活动。
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收有图像活动的视频输出帧包。视频输出帧时间戳：" + p_TmpInt + "。总长度：" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "。" );
                                        }
                                        else //如果该视频输出帧为无图像活动。
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收无图像活动的视频输出帧包。视频输出帧时间戳：" + p_TmpInt + "。总长度：" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "。" );
                                        }
                                    }
                                    else if( m_ClntMediaPocsThrdPt.m_TmpBytePt[ 0 ] == ( byte ) ClntMediaPocsThrd.PktTyp.Exit ) //如果是退出包。
                                    {
                                        ...
                                        {
                                            if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收退出包。长度为" + m_ClntMediaPocsThrdPt.m_TmpHTLongPt.m_Val + "小于1 + 1，表示没有对讲索引，无法继续接收。" );
                                            break RecvPktOut;
                                        }

                                        String p_InfoStrPt = "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：接收到一个退出包。";
                                        if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, p_InfoStrPt );
                                        m_ClntMediaPocsThrdPt.UserShowLog( p_InfoStrPt );
                                        if( m_ClntMediaPocsThrdPt.m_IsShowToast != 0 ) m_ClntMediaPocsThrdPt.UserShowToast( p_InfoStrPt );

                                        if( m_ClntMediaPocsThrdPt.m_TmpBytePt[ 1 ] == p_CnctInfoTmpPt.m_MyTkbkIdx ) //如果对讲索引是我的对讲索引。
                                        {
                            ...
                            {
                                String p_InfoStrPt = "客户端媒体处理线程：广播客户端：连接" + p_CnctInfoTmpPt.m_Idx + "：用本端套接字接收一个连接的远端套接字发送的数据包失败。原因：" + m_ClntMediaPocsThrdPt.m_ErrInfoVstrPt.GetStr();
                                if( m_ClntMediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( m_ClntMediaPocsThrdPt.m_CurClsNameStrPt, p_InfoStrPt );
                                m_ClntMediaPocsThrdPt.UserShowLog( p_InfoStrPt );
                                       ...
                                       byte VdoInptEncdRsltFrmPt[], long VdoInptEncdRsltFrmLenByt )
    {
        ...
        if( AdoInptPcmSrcFrmPt != null ) //如果有音频输入Pcm格式原始帧。
        {
            ...
            if( p_PktLenByt != 1 + 1 + 4 ) //如果本次音频输入帧为有语音活动，就发送。
            {
                ...
                AllCnctSendAdoPkt( m_ClntMediaPocsThrdPt.m_TmpBytePt, p_PktLenByt, 1, 0 );
