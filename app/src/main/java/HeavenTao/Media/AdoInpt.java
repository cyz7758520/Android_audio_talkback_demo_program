...
public class AdoInpt //音频输入类。
{
			...
			switch( m_UseWhatAec )
			{
				...
				case 1: //如果要使用Speex声学回音消除器。
				{
					if( m_SpeexAecIsSaveMemFile != 0 )
					{
						...
						if( m_SpeexAecPt.InitByMemFile( m_SmplRate, m_FrmLen, m_SpeexAecFilterLen, m_SpeexAecIsUseRec, m_SpeexAecEchoMultiple, m_SpeexAecEchoCont, m_SpeexAecEchoSupes, m_SpeexAecEchoSupesAct, m_SpeexAecMemFileFullPathStrPt, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
						{
					...
					{
						m_SpeexAecPt = new SpeexAec();
						if( m_SpeexAecPt.Init( m_SmplRate, m_FrmLen, m_SpeexAecFilterLen, m_SpeexAecIsUseRec, m_SpeexAecEchoMultiple, m_SpeexAecEchoCont, m_SpeexAecEchoSupes, m_SpeexAecEchoSupesAct, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
						{
						...
						{
							m_SpeexAecPt = null;
							String asd = m_MediaPocsThrdPt.m_ErrInfoVstrPt.GetStr();
		...
		switch( m_UseWhatAec )
		{
			...
			case 1: //如果要使用Speex声学回音消除器。
			{
				if( m_SpeexAecPt != null )
				{
					if( m_SpeexAecIsSaveMemFile != 0 )
					{
						if( m_SpeexAecPt.SaveMemFile( m_SmplRate, m_FrmLen, m_SpeexAecFilterLen, m_SpeexAecIsUseRec, m_SpeexAecEchoMultiple, m_SpeexAecEchoCont, m_SpeexAecEchoSupes, m_SpeexAecEchoSupesAct, m_SpeexAecMemFileFullPathStrPt, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
						{
	...
	void SetIsCanUseAec()
	{
		if( m_UseWhatAec != 0 ) //如果要使用声学回音消除器。
		{
			...
			{
				if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_MediaPocsThrdPt.m_CurClsNameStrPt, "媒体处理线程：不使用音频输入时，不能使用声学回音消除器。" );
				m_IsCanUseAec = 0; //设置是否可以使用声学回音消除器为不可以。
			}
			else if( m_MediaPocsThrdPt.m_AdoOtptPt.m_IsUseAdoOtpt == 0 )
			{
				if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_MediaPocsThrdPt.m_CurClsNameStrPt, "媒体处理线程：不使用音频输出时，不能使用声学回音消除器。" );
				m_IsCanUseAec = 0; //设置是否可以使用声学回音消除器为不可以。
			}
			else if( m_MediaPocsThrdPt.m_AdoOtptPt.m_SmplRate != m_SmplRate )
			{
				if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_MediaPocsThrdPt.m_CurClsNameStrPt, "媒体处理线程：音频输出的采样频率与音频输入不一致时，不能使用声学回音消除器。" );
				m_IsCanUseAec = 0; //设置是否可以使用声学回音消除器为不可以。
			}
			else if( m_MediaPocsThrdPt.m_AdoOtptPt.m_FrmLen != m_FrmLen )
			{
				if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( m_MediaPocsThrdPt.m_CurClsNameStrPt, "媒体处理线程：音频输出帧的长度与音频输入不一致时，不能使用声学回音消除器。" );
				m_IsCanUseAec = 0; //设置是否可以使用声学回音消除器为不可以。
			}
			else
			{
				m_IsCanUseAec = 1; //设置是否可以使用声学回音消除器为可以。
			}
		}
		else
		{
	...
	public int SpeexPrpocsInit()
	{
		...
		Out:
		{
			if( ( m_UseWhatNs == 1 ) || ( m_IsUseSpeexPrpocsOther != 0 ) )
			{
				if( m_UseWhatNs != 1 )
				{
					m_SpeexPrpocsIsUseNs = 0;
					m_SpeexPrpocsIsUseDereverb = 0;
				}
				if( m_IsUseSpeexPrpocsOther == 0 )
				{
					m_SpeexPrpocsIsUseVad = 0;
					m_SpeexPrpocsIsUseAgc = 0;
				}
				m_SpeexPrpocsPt = new SpeexPrpocs();
				if( m_SpeexPrpocsPt.Init( m_SmplRate, m_FrmLen, m_SpeexPrpocsIsUseNs, m_SpeexPrpocsNoiseSupes, m_SpeexPrpocsIsUseDereverb, m_SpeexPrpocsIsUseVad, m_SpeexPrpocsVadProbStart, m_SpeexPrpocsVadProbCont, m_SpeexPrpocsIsUseAgc, m_SpeexPrpocsAgcLevel, m_SpeexPrpocsAgcIncrement, m_SpeexPrpocsAgcDecrement, m_SpeexPrpocsAgcMaxGain, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
				{
	...
	public int EncdInit()
	{
		...
		Out:
		{
			switch( m_UseWhatEncd )
			{
				case 0: //如果要使用PCM原始数据。
				{
					if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：使用PCM原始数据。" );
					break;
				}
	...
	public class AdoInptThrd extends Thread
	{
		public void run()
		{
			...
			if( m_IsCanUseAec != 0 ) //如果可以使用声学回音消除器，就自适应计算声学回音的延迟，并设置到声学回音消除器。放在音频输入线程中计算，可以减少媒体处理线程的初始化时间。
			{
				...
				m_MediaPocsThrdPt.m_AdoOtptPt.m_AdoOtptThrdPt.start(); //启动音频输出线程。
			...
			{
				m_AdoInptDvcPt.startRecording(); //让音频输入设备开始录音。
				if( m_MediaPocsThrdPt.m_AdoOtptPt.m_IsUseAdoOtpt != 0 ) //如果要使用音频输出。
				{
					m_MediaPocsThrdPt.m_AdoOtptPt.m_AdoOtptDvcPt.play(); //让音频输出设备开始播放。
					m_MediaPocsThrdPt.m_AdoOtptPt.m_AdoOtptThrdPt.start(); //启动音频输出线程。
				}
