...
public class AdoInpt //音频输入。
{
    ...
    SurfaceView m_AdoInptSrcWavfmSurfacePt; //存放音频输入原始波形Surface的指针。
    AdoWavfm m_AdoInptSrcWavfmPt; //存放音频输入原始波形器的指针。
    SurfaceView m_AdoRsltWavfmSurfacePt; //存放音频输入结果波形Surface的指针。
    AdoWavfm m_AdoRsltWavfmPt; //存放音频输入结果波形器的指针。
    ...
    String m_AdoInptSrcWaveFileFullPathStrPt; //存放音频输入原始Wave文件完整路径字符串的指针。
    String m_AdoInptRsltWaveFileFullPathStrPt; //存放音频输入结果Wave文件完整路径字符串的指针。
    long m_AdoInptFileWrBufSzByt; //存放音频输入文件写入缓冲区的大小，单位为字节。

    AudioRecord m_AdoInptDvcPt; //存放音频输入设备的指针。
    int m_AdoInptDvcBufSzByt; //存放音频输入设备缓冲区大小，单位为字节。
    public int m_AdoInptIsMute; //存放音频输入是否静音，为0表示有声音，为非0表示静音。

    public LinkedList< short[] > m_PcmAdoInptSrcFrmLnkLstPt; //存放Pcm格式音频输入原始帧链表的指针。
    public LinkedList< short[] > m_PcmAdoInptIdleFrmLnkLstPt; //存放Pcm格式Pcm格式音频输入空闲帧链表的指针。
            ...
            switch( m_UseWhatAec )
            {
                ...
                case 1: //如果要使用Speex声学回音消除器。
                {
                    if( m_SpeexAecIsSaveMemFile != 0 ) //如果Speex声学回音消除器要保存内存块到文件。
                    {
                        ...
                        if( m_SpeexAecPt.InitByMemFile( m_SmplRate, m_FrmLenUnit, m_SpeexAecFilterLen, m_SpeexAecIsUseRec, m_SpeexAecEchoMutp, m_SpeexAecEchoCntu, m_SpeexAecEchoSupes, m_SpeexAecEchoSupesAct, m_SpeexAecMemFileFullPathStrPt, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                        {
                    ...
                    {
                        m_SpeexAecPt = new SpeexAec();
                        if( m_SpeexAecPt.Init( m_SmplRate, m_FrmLenUnit, m_SpeexAecFilterLen, m_SpeexAecIsUseRec, m_SpeexAecEchoMutp, m_SpeexAecEchoCntu, m_SpeexAecEchoSupes, m_SpeexAecEchoSupesAct, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                        {
                ...
                case 2: //如果要使用WebRtc定点版声学回音消除器。
                {
                    ...
                    if( m_WebRtcAecmPt.Init( m_SmplRate, m_FrmLenUnit, m_WebRtcAecmIsUseCNGMode, m_WebRtcAecmEchoMode, m_WebRtcAecmDelay, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                    {
                ...
                case 3: //如果要使用WebRtc浮点版声学回音消除器。
                {
                    if( m_WebRtcAecIsSaveMemFile != 0 ) //如果WebRtc浮点版声学回音消除器要保存内存块到文件。
                    {
                        ...
                        if( m_WebRtcAecPt.InitByMemFile( m_SmplRate, m_FrmLenUnit, m_WebRtcAecEchoMode, m_WebRtcAecDelay, m_WebRtcAecIsUseDelayAgstcMode, m_WebRtcAecIsUseExtdFilterMode, m_WebRtcAecIsUseRefinedFilterAdaptAecMode, m_WebRtcAecIsUseAdaptAdjDelay, m_WebRtcAecMemFileFullPathStrPt, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                        {
                    ...
                    {
                        m_WebRtcAecPt = new WebRtcAec();
                        if( m_WebRtcAecPt.Init( m_SmplRate, m_FrmLenUnit, m_WebRtcAecEchoMode, m_WebRtcAecDelay, m_WebRtcAecIsUseDelayAgstcMode, m_WebRtcAecIsUseExtdFilterMode, m_WebRtcAecIsUseRefinedFilterAdaptAecMode, m_WebRtcAecIsUseAdaptAdjDelay, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                        {
        ...
        switch( m_UseWhatAec )
        {
            ...
            case 1: //如果要使用Speex声学回音消除器。
            {
                if( m_SpeexAecPt != null )
                {
                    if( m_SpeexAecIsSaveMemFile != 0 ) //如果Speex声学回音消除器要保存内存块到文件。
                    {
                        if( m_SpeexAecPt.SaveMemFile( m_SmplRate, m_FrmLenUnit, m_SpeexAecFilterLen, m_SpeexAecIsUseRec, m_SpeexAecEchoMutp, m_SpeexAecEchoCntu, m_SpeexAecEchoSupes, m_SpeexAecEchoSupesAct, m_SpeexAecMemFileFullPathStrPt, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                        {
            ...
            case 3: //如果要使用WebRtc浮点版声学回音消除器。
            {
                if( m_WebRtcAecPt != null )
                {
                    if( m_WebRtcAecIsSaveMemFile != 0 ) //如果WebRtc浮点版声学回音消除器要保存内存块到文件。
                    {
                        if( m_WebRtcAecPt.SaveMemFile( m_SmplRate, m_FrmLenUnit, m_WebRtcAecEchoMode, m_WebRtcAecDelay, m_WebRtcAecIsUseDelayAgstcMode, m_WebRtcAecIsUseExtdFilterMode, m_WebRtcAecIsUseRefinedFilterAdaptAecMode, m_WebRtcAecIsUseAdaptAdjDelay, m_WebRtcAecMemFileFullPathStrPt, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                        {
        ...
        Out:
        {
            switch( m_UseWhatNs )
            {
                ...
                case 2: //如果要使用WebRtc定点版噪音抑制器。
                {
                    ...
                    if( m_WebRtcNsxPt.Init( m_SmplRate, m_FrmLenUnit, m_WebRtcNsxPolicyMode, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                    {
                ...
                case 3: //如果要使用WebRtc浮点版噪音抑制器。
                {
                    ...
                    if( m_WebRtcNsPt.Init( m_SmplRate, m_FrmLenUnit, m_WebRtcNsPolicyMode, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                    {
                ...
                case 4: //如果要使用RNNoise噪音抑制器。
                {
                    ...
                    if( m_RNNoisePt.Init( m_SmplRate, m_FrmLenUnit, m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                    {
    ...
    public int WavfmInit()
    {
        ...
        Out:
        {
            ...
            {
                m_AdoInptSrcWavfmPt = new AdoWavfm();
                if( m_AdoInptSrcWavfmPt.Init( m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入原始波形器成功。" );
                }
                else
                {
                    m_AdoInptSrcWavfmPt = null;
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入原始波形器失败。原因：" + m_MediaPocsThrdPt.m_ErrInfoVstrPt.GetStr() );
                    break Out;
                }
                m_AdoRsltWavfmPt = new AdoWavfm();
                if( m_AdoRsltWavfmPt.Init( m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入结果波形器成功。" );
                }
                else
                {
                    m_AdoRsltWavfmPt = null;
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入结果波形器失败。原因：" + m_MediaPocsThrdPt.m_ErrInfoVstrPt.GetStr() );
                    break Out;
                }
            }
    ...
    //销毁音频输入原始波形器、音频输入结果波形器。
    public void WavfmDstoy()
    {
        if( m_IsDrawAdoWavfmToSurface != 0 )
        {
            if( m_AdoInptSrcWavfmPt != null )
            {
                if( m_AdoInptSrcWavfmPt.Dstoy( m_MediaPocsThrdPt.m_ErrInfoVstrPt ) == 0 )
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁音频输入原始波形器成功。" );
                }
                else
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁音频输入原始波形器失败。原因：" + m_MediaPocsThrdPt.m_ErrInfoVstrPt.GetStr() );
                }
                m_AdoInptSrcWavfmPt = null;
            }
            if( m_AdoRsltWavfmPt != null )
            {
                ...
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁音频输入结果波形器成功。" );
                }
                else
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁音频输入结果波形器失败。原因：" + m_MediaPocsThrdPt.m_ErrInfoVstrPt.GetStr() );
                }
                m_AdoRsltWavfmPt = null;
            }
        }
    ...
    //初始化音频输入原始Wave文件写入器、音频输入结果Wave文件写入器。
    public int WaveFileWriterInit()
    {
        ...
        Out:
        {
            ...
            {
                m_AdoInptSrcWaveFileWriterPt = new WaveFileWriter();
                if( m_AdoInptSrcWaveFileWriterPt.Init( m_AdoInptSrcWaveFileFullPathStrPt, m_AdoInptFileWrBufSzByt, ( short ) 1, m_SmplRate, 16 ) == 0 )
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入原始Wave文件 " + m_AdoInptSrcWaveFileFullPathStrPt + " 的Wave文件写入器成功。" );
                }
                else
                {
                    m_AdoInptSrcWaveFileWriterPt = null;
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入原始Wave文件 " + m_AdoInptSrcWaveFileFullPathStrPt + " 的Wave文件写入器失败。" );
                    break Out;
                }
                m_AdoInptRsltWaveFileWriterPt = new WaveFileWriter();
                if( m_AdoInptRsltWaveFileWriterPt.Init( m_AdoInptRsltWaveFileFullPathStrPt, m_AdoInptFileWrBufSzByt, ( short ) 1, m_SmplRate, 16 ) == 0 )
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入结果Wave文件 " + m_AdoInptRsltWaveFileFullPathStrPt + " 的Wave文件写入器成功。" );
                }
                else
                {
                    m_AdoInptRsltWaveFileWriterPt = null;
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入结果Wave文件 " + m_AdoInptRsltWaveFileFullPathStrPt + " 的Wave文件写入器失败。" );
                    break Out;
                }
            }
    ...
    //销毁音频输入原始Wave文件写入器、音频输入结果Wave文件写入器。
    public void WaveFileWriterDstoy()
    {
        if( m_IsSaveAdoToWaveFile != 0 )
        {
            if( m_AdoInptSrcWaveFileWriterPt != null )
            {
                if( m_AdoInptSrcWaveFileWriterPt.Dstoy() == 0 )
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁音频输入原始Wave文件写入器成功。" );
                }
                else
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁音频输入原始Wave文件写入器失败。" );
                }
                m_AdoInptSrcWaveFileWriterPt = null;
            }
            if( m_AdoInptRsltWaveFileWriterPt != null )
            {
                if( m_AdoInptRsltWaveFileWriterPt.Dstoy() == 0 )
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁音频输入结果Wave文件写入器成功。" );
                }
                else
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁音频输入结果Wave文件写入器失败。" );
                }
                m_AdoInptRsltWaveFileWriterPt = null;
            }
        }
    ...
    public int DvcAndThrdInit()
    {
        ...
        Out:
        {
            ...
            try
            {
                m_AdoInptDvcBufSzByt = AudioRecord.getMinBufferSize( m_SmplRate, AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_16BIT );
                m_AdoInptDvcBufSzByt = ( m_AdoInptDvcBufSzByt > m_FrmLenByt ) ? m_AdoInptDvcBufSzByt : m_FrmLenByt;
                m_AdoInptDvcPt = new AudioRecord(
                        ...
                        m_AdoInptDvcBufSzByt );
                if( m_AdoInptDvcPt.getState() == AudioRecord.STATE_INITIALIZED )
                {
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入设备成功。音频输入设备缓冲区大小：" + m_AdoInptDvcBufSzByt );
                }
                else
                {
            ...
            //初始化Pcm格式音频输入原始帧链表。
            m_PcmAdoInptSrcFrmLnkLstPt = new LinkedList< short[] >();
            if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化Pcm格式音频输入原始帧链表成功。" );

            //初始化Pcm格式音频输入空闲帧链表。
            m_PcmAdoInptIdleFrmLnkLstPt = new LinkedList< short[] >();
            if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化Pcm格式音频输入空闲帧链表成功。" );

            //初始化音频输入线程的临时变量。
            {
                m_IsInitAdoInptThrdTmpVar = 1; //设置已初始化音频输入线程的临时变量。
                m_PcmAdoInptSrcFrmPt = null; //初始化Pcm格式音频输入原始帧的指针。
    ...
    public void DvcAndThrdDstoy()
    {
        ...
        if( m_IsInitAdoInptThrdTmpVar != 0 )
        {
            ...
            m_PcmAdoInptSrcFrmPt = null; //销毁Pcm格式音频输入原始帧的指针。
        ...
        //销毁Pcm格式音频输入空闲帧链表。
        if( m_PcmAdoInptIdleFrmLnkLstPt != null )
        {
            m_PcmAdoInptIdleFrmLnkLstPt.clear();
            m_PcmAdoInptIdleFrmLnkLstPt = null;
            if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁Pcm格式音频输入空闲帧链表成功。" );
        }

        //销毁Pcm格式音频输入原始帧链表。
        if( m_PcmAdoInptSrcFrmLnkLstPt != null )
        {
            m_PcmAdoInptSrcFrmLnkLstPt.clear();
            m_PcmAdoInptSrcFrmLnkLstPt = null;
            if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：销毁Pcm格式音频输入原始帧链表成功。" );
        }
    ...
    public int Init()
    {
        ...
        Out:
        {
            ...
            if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 )
            {
                ...
                Log.i( MediaPocsThrd.m_CurClsNameStrPt, "媒体处理线程：初始化音频输入耗时 " + ( p_NowTickMsec - p_LastTickMsec ) + " 毫秒。" );
            }
    ...
    public class AdoInptThrd extends Thread
    {
        public void run()
        {
            ...
            if( m_IsCanUseAec != 0 ) //如果可以使用声学回音消除器，就自适应计算声学回音的延迟，并设置到声学回音消除器。放在音频输入线程中计算，可以减少媒体处理线程的初始化时间。
            {
                ...
                m_PcmAdoInptSrcFrmPt = new short[ m_MediaPocsThrdPt.m_AdoOtptPt.m_SmplRate / 1000 * 10 ]; //创建一个10ms空的音频输出帧。
                m_LastTickMsec = SystemClock.uptimeMillis();
                while( true )
                {
                    m_MediaPocsThrdPt.m_AdoOtptPt.m_AdoOtptDvcPt.write( m_PcmAdoInptSrcFrmPt, 0, m_PcmAdoInptSrcFrmPt.length ); //播放一个空的音频输出帧。
                    m_NowTickMsec = SystemClock.uptimeMillis();
                    p_AdoOtptDelay += 10; //递增音频输出延迟。
                    if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "音频输入线程：" + "本次音频输出耗时 " + ( m_NowTickMsec - m_LastTickMsec ) + " 毫秒，音频输出延迟 " + p_AdoOtptDelay + " 毫秒。" );
                    if( m_NowTickMsec - m_LastTickMsec >= 10 ) //如果播放耗时较长，就表示音频输出设备的缓冲区已经写满，结束计算。
                    {
                        break;
                    }
                    m_LastTickMsec = m_NowTickMsec;
                }
            ...
            while( true )
            {
                ...
                {
                    //获取一个Pcm格式音频输入空闲帧。
                    m_FrmLnkLstElmTotal = m_PcmAdoInptIdleFrmLnkLstPt.size(); //获取Pcm格式音频输入空闲帧链表的元素总数。
                    if( m_FrmLnkLstElmTotal > 0 ) //如果Pcm格式音频输入空闲帧链表中有帧。
                    {
                        //从Pcm格式音频输入空闲帧链表中取出第一个帧。
                        synchronized( m_PcmAdoInptIdleFrmLnkLstPt )
                        {
                            m_PcmAdoInptSrcFrmPt = m_PcmAdoInptIdleFrmLnkLstPt.getFirst();
                            m_PcmAdoInptIdleFrmLnkLstPt.removeFirst();
                        }
                        if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "音频输入线程：从Pcm格式音频输入空闲帧链表中取出第一个帧，Pcm格式音频输入空闲帧链表元素总数：" + m_FrmLnkLstElmTotal + "。" );
                    }
                    else //如果Pcm格式音频输入空闲帧链表中没有帧。
                    {
                        m_FrmLnkLstElmTotal = m_PcmAdoInptSrcFrmLnkLstPt.size(); //获取Pcm格式音频输入原始帧链表的元素总数。
                        if( m_FrmLnkLstElmTotal <= 50 )
                        {
                            m_PcmAdoInptSrcFrmPt = new short[ m_FrmLenUnit ]; //创建一个音频输入空闲帧。
                            if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.i( MediaPocsThrd.m_CurClsNameStrPt, "音频输入线程：Pcm格式音频输入空闲帧链表中没有帧，创建一个Pcm格式音频输入空闲帧成功。" );
                        }
                        else
                        {
                            if( m_MediaPocsThrdPt.m_IsPrintLogcat != 0 ) Log.e( MediaPocsThrd.m_CurClsNameStrPt, "音频输入线程：Pcm格式音频输入原始帧链表中帧总数为" + m_FrmLnkLstElmTotal + "已经超过上限50，不再创建Pcm格式音频输入空闲帧。" );
                    ...
                    //读取本次音频输入帧。
                    m_AdoInptDvcPt.read( m_PcmAdoInptSrcFrmPt, 0, m_PcmAdoInptSrcFrmPt.length );

                    //追加本次Pcm格式音频输入原始帧到Pcm格式音频输入原始帧链表。
                    synchronized( m_PcmAdoInptSrcFrmLnkLstPt )
                    {
                        m_PcmAdoInptSrcFrmLnkLstPt.addLast( m_PcmAdoInptSrcFrmPt );
                    }
                    m_PcmAdoInptSrcFrmPt = null;
                    ...
                    {
                        m_NowTickMsec = SystemClock.uptimeMillis();
                        Log.i( MediaPocsThrd.m_CurClsNameStrPt, "音频输入线程：本次音频输入帧处理完毕，耗时 " + ( m_NowTickMsec - m_LastTickMsec ) + " 毫秒。" );
                    }
                }
