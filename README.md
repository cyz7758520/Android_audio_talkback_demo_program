...
###### Speex预处理器对应：libFunc.so、libSpeexDsp.so、SpeexPrpocs.java。
###### Speex编解码器对应：libFunc.so、libSpeex.so、SpeexEncd.java、SpeexDecd.java。
###### Wave文件写入读取器对应：libFunc.so、libWaveFile.so、WaveFileReader.java、WaveFileWriter.java。
###### 音频波形器对应：libFunc.so、libAdoWavfm.so、AdoWavfm.java。
###### OpenH264编解码器对应：libFunc.so、libOpenH264.so、OpenH264Encd.java、OpenH264Decd.java。
###### 系统自带H264编解码器对应：libFunc.so、libSystemH264.so、SystemH264Encd.java、SystemH264Decd.java。
